...
import { useAllTestGuides, useCreateTestGuide, useUpdateTestGuide, useDeleteTestGuide } from "@/hooks/useTestGuides";
...
export default function TestGuidesTab() {
  ...
  const [blocks, setBlocks] = useState<GuideBlock[]>([
    { name: "Bloco 1", questions: [], criteria: [] },
  ]);

  const { data: guides, isLoading } = useAllTestGuides(filterJob === "all" ? undefined : filterJob);
  const { data: jobs } = useJobs();
  const createGuide = useCreateTestGuide();
  const updateGuide = useUpdateTestGuide();
  const deleteGuide = useDeleteTestGuide();

  const resetForm = () => {
    setEditingId(null);
    setSelectedJobId("");
    setIsActive(true);
    setBlocks([{ name: "Bloco 1", questions: [], criteria: [] }]);
  };
  ...
  const openEdit = (guide: any) => {
    ...
    const rawBlocks: GuideBlock[] = json.blocks?.length ? json.blocks : [{ name: "Bloco 1", questions: [], criteria: [] }];
  ...
  const addBlock = () => setBlocks([...blocks, { name: "", questions: [], criteria: [] }]);
  ...
  return (
    <div className="space-y-4">
        ...
        <Dialog open={dialogOpen} onOpenChange={(v) => { setDialogOpen(v); if (!v) resetForm(); }}>
          {canManage && (
            <DialogTrigger asChild>
              <Button size="sm"><Plus className="h-4 w-4 mr-1" />Novo Roteiro de Teste</Button>
            </DialogTrigger>
          )}
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{!canManage ? "Visualizar Roteiro" : editingId ? "Editar Roteiro de Teste" : "Novo Roteiro de Teste"}</DialogTitle></DialogHeader>
            {!canManage ? (
              <div className="space-y-4">
            ...
            <fieldset className="space-y-4">
              ...
              {canManage && (
                <Button className="w-full" onClick={handleSave} disabled={createGuide.isPending || updateGuide.isPending}>
                  ...
                  {editingId ? "Atualizar" : "Criar Roteiro de Teste"}
                </Button>
              )}
            </fieldset>
            )}
      ...
      <Card>
        <CardHeader><CardTitle>Roteiros de Teste Cadastrados</CardTitle><CardDescription>Lista de roteiros de teste por cargo</CardDescription></CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="flex justify-center p-4"><Loader2 className="h-6 w-6 animate-spin" /></div>
          ) : !guides?.length ? (
            <p className="text-muted-foreground text-sm text-center py-4">Nenhum roteiro de teste cadastrado.</p>
          ) : (
                      ...
                      <TableCell className="flex gap-1">
                        ...
                        {canManage && (
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                <AlertDialogTitle>Excluir roteiro de teste</AlertDialogTitle>
                                <AlertDialogDescription>
                                  Tem certeza que deseja excluir este roteiro de teste? Esta ação não pode ser desfeita.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction
                                  onClick={async () => {
                                    try {
                                      ...
                                      toast.success("Roteiro de teste excluído!");
                                    } catch (e: any) {
                                      toast.error(e.message);
                                    }
                                  }}
