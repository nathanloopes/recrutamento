...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const giovanna = await safe(() => sql`SELECT id, full_name, cpf, email, phone, created_at, status FROM candidates WHERE full_name ILIKE '%giovanna%' OR full_name ILIKE '%giammarino%' OR email ILIKE '%giammarino%' ORDER BY created_at DESC LIMIT 20`);
    const apps = await safe(() => sql`SELECT a.id, a.candidate_id, a.status, a.score, a.created_at, c.full_name, c.email, c.cpf, c.phone FROM applications a LEFT JOIN candidates c ON c.id=a.candidate_id WHERE a.unit_job_id='ea6421d6-9d2c-4450-a1d2-f33461940b52' ORDER BY a.created_at DESC`);
    ...
    const journeyCount = await safe(() => sql`SELECT count(*) FROM application_journey_events e JOIN applications a ON a.id=e.application_id WHERE a.unit_job_id='ea6421d6-9d2c-4450-a1d2-f33461940b52'`);
    const links = await safe(() => sql`SELECT * FROM recruitment_links WHERE unit_job_id='ea6421d6-9d2c-4450-a1d2-f33461940b52'`);
    const exclLog = await safe(() => sql`SELECT created_at, user_id, action, metadata FROM activity_logs WHERE action ILIKE '%vaga_corporativa_excluida%' OR metadata::text ILIKE '%ea6421d6%' ORDER BY created_at DESC LIMIT 10`);

    await sql.end();
    return new Response(JSON.stringify({
      used_host: used, snapshot, socialJob, giovanna, apps, interviews, journeyCount, links, exclLog,
