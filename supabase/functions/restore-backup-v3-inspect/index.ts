...
const REF = 'fxbbjjuklxfsxdzpmuue';
const HOSTS = [
...
Deno.serve(async (req) => {
  ...
  const password = Deno.env.get('BACKUP_DB_PASSWORD_V3');
  if (!password) return new Response(JSON.stringify({ error: 'missing BACKUP_DB_PASSWORD_V3' }), {
  ...
  try {
    ...
    const recentApps = await safe(() => sql`
      SELECT a.id, a.candidate_id, a.unit_job_id, a.status, a.score, a.created_at, a.updated_at,
    ...
    const links = await safe(() => sql`
      SELECT rl.id, rl.code, rl.unit_job_id, rl.created_at, rl.clicks_count,
             j.title as job_title, u.name as unit_name
